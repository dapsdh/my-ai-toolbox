...
위 입력에서 **Jira 필터 페이지 URL**을 추출해 아래 워크플로를 따른다.
...
2. **필터 JQL 조회**
   `python .claude/skills/jira-filter-summarizer/scripts/jira/summarize_jira_filter.py <필터_URL_또는_filter_ID>` 를 실행한다.
   스크립트는 내부적으로:
...
```bash
python .claude/skills/jira-filter-summarizer/scripts/jira/summarize_jira_filter.py "https://example.atlassian.net/issues/?filter=12345"
# 또는
python .claude/skills/jira-filter-summarizer/scripts/jira/summarize_jira_filter.py 12345
```
