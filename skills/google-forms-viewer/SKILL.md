...
- Google 인증용 `.env`는 **`.cursor/skills/google-forms-viewer/.env`** 에 둔다. (없으면 프로젝트 루트 `.env` 사용)
- **방법 1 (서비스 계정 권장)**  
...
- **방법 2 (OAuth 리프레시 토큰)**  
  ...
  - 한 번 OAuth 흐름을 실행해 리프레시 토큰을 발급받아 둔다.
- 인증이 없으면 ".env에 Google 인증 정보를 설정해 주세요" 및 아래 설정 방법 안내.
...
2. **스크립트 실행**  
   ...
   ```  
   스크립트는:
   - .env에서 인증 정보 로드 후 Google Forms API 호출.
...
- API 오류·인증 실패 시 stderr 메시지와 exit code 1로 종료. 사용자에게 .env 설정 및 폼 공유 안내.
