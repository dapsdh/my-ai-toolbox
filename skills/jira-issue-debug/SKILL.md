...
description: Accepts a Jira issue URL plus "문제코드를 찾아줘" or "디버깅해줘", then analyzes the issue description, finds problem code if it is a fixable bug, and suggests fixes. Use when the user pastes a Jira issue link and asks to find problem code or debug ("문제코드를 찾아줘", "디버깅해줘"). Uses this skill's .env for Jira auth.
---
...
예: `https://hancom.atlassian.net/browse/PROJ-123 문제코드를 찾아줘`
...
- **인증**: 이 스킬의 `.env` (ATLASSIAN_USER, ATLASSIAN_API_TOKEN, ATLASSIAN_BASE_URL).
