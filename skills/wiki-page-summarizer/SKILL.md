...
- **저장 위치**: **이 스킬 디렉터리의 `.env`**. `.env.example`을 복사해 `.env`로 저장한 뒤 값을 채운다.
- **필수 변수**:
  - `ATLASSIAN_USER`: Jira 로그인 이메일 (예: sanghee@hancom.com)
  - `ATLASSIAN_API_TOKEN`: [Atlassian API 토큰](https://id.atlassian.com/manage-profile/security/api-tokens) (비밀번호가 아님, 사이트에서 발급)
- **선택 변수**: `ATLASSIAN_BASE_URL` (기본값: `https://hancom.atlassian.net`)

인증이 필요한데 `.env`가 없거나 토큰이 비어 있으면, 사용자에게 ".env에 ATLASSIAN_USER(이메일)와 ATLASSIAN_API_TOKEN을 설정해 주세요"라고 안내한다.
...
2. **결과 판단**
   ...
   - **로그인 페이지 / 401·403 / JavaScript 로그인 요구** → **인증 경로 사용**:
     ...
     - **Confluence URL** (예: `.../wiki/.../pages/1997013274/...`): 페이지 ID 추출 후  
       ...
       `--with-children` 사용 시 전달한 링크의 **하위 폴더를 포함해 재귀적으로 모든 페이지**를 가져온다. 스크립트는 이 스킬 디렉터리의 `.env`의 `ATLASSIAN_USER`, `ATLASSIAN_API_TOKEN`, `ATLASSIAN_BASE_URL`을 사용하며, 성공 시 출력(본문 + 모든 하위·손자 페이지)을 2단계 입력으로 사용해 정리한다.
     - 스크립트 실패(인증 오류 등) 시: ".env에 ATLASSIAN_USER(이메일)와 ATLASSIAN_API_TOKEN이 올바른지 확인해 주세요. API 토큰은 https://id.atlassian.com/manage-profile/security/api-tokens 에서 발급할 수 있습니다." 안내. 그래도 안 되면 **본문을 복사해 붙여넣어 달라**고 요청한다.
...
- **API 토큰·비밀번호**: 코드나 스킬 파일에 절대 넣지 않고, `.env` 또는 환경 변수로만 참조한다.
